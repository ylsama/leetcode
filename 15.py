...
class Solution:
    ...
    # because result triplet is not duplicate, so there could be some
    # independent on handling input
    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in nums:
            ...
            nums_dict[i] += 1
        result = set()
        # I also using set, avoiding writing handling code to check the triplet
        # is duplicate
        # This also mean the item need to hashable. so i will use Tupple
        #       1. The tupple in python is index dependent, so i will need to
        #       sort the triplet before adding to result_set
        #
...
if __name__ == "__main__":
    ...
    # 3 <= nums.length <= 3000
    # -10**5 <= nums[i] <= 10**5
    Output = a.threeSum(nums=[random.randint(-10**5, 10**5)
                        for i in range(3000)])
